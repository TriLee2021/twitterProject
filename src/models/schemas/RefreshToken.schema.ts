...
interface RefreshTokenType {
  ...
  user_id: ObjectId //đối tượng sở hữu cái token đó
}
...
export default class RefreshToken {
  ...
  constructor({ _id, token, created_at, user_id }: RefreshTokenType) {
    this._id = _id //quan trọng
    this.token = token //Mongo tự tạo
    this.created_at = created_at || new Date() //Mongo tự tạo
    this.user_id = user_id //quan trọng
  }
}
